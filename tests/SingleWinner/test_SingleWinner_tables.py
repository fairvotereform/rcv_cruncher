    ...
    ({
        ...
        'expected': {
            ...
            'tables': [
                ...
                pd.DataFrame({
                    'A': [util.NAN, round(100*3/13, 3), round(100*3/10, 3), round(100*0/10, 3), round(100*6/13, 3)],
                    'B': [round(100*10/13, 3), util.NAN, round(100*6/11, 3), round(100*0/8, 3), round(100*8/13, 3)],
                    'C': [round(100*7/10, 3), round(100*5/11, 3), util.NAN, round(100*0/8, 3), round(100*8/13, 3)],
                    'D': [round(100*10/10, 3), round(100*8/8, 3), round(100*8/8, 3), util.NAN, round(100*8/10, 3)],
                    BallotMarks.WRITEIN: [round(100*7/13, 3), round(100*5/13, 3), round(100*5/13, 3), round(100*2/10, 3), util.NAN]
                }, index=['A', 'B', 'C', 'D', BallotMarks.WRITEIN], dtype=float)
            ]
        }
    }),
...
def test_condorcet_tables(param):
    ...
    count_df, percent_df, cwinner = rcv.condorcet_tables()
    ...
    ({
        ...
        'expected': {
            'tables': [
                ...
                }, index=['first_choice', 'A', 'B', 'C', 'D', BallotMarks.WRITEIN, 'exhaust'], dtype=float),
                pd.DataFrame({
                ...
                }, index=['first_choice', 'A', 'B', 'C', 'D', BallotMarks.WRITEIN], dtype=float)
            ]
        }
    }),
]
...
def test_first_second_tables(param):
    ...
    count_df, percent_df, percent_no_exhaust_df = rcv.first_second_tables()
    ...
    assert percent_no_exhaust_df.fillna('NA').to_dict('records') == param['expected']['tables'][2].fillna('NA').to_dict('records')
...
def test_cumulative_ranking_table(param):
    ...
    count_df, percent_df = rcv.cumulative_ranking_tables()
...
params = [
    ({
        ...
        'expected': {
            'tables': [
                pd.DataFrame({
                    'candidate': ['A', BallotMarks.WRITEIN, 'C', 'B', 'D', 'exhaust', 'colsum'],
                    ...
                    'r2_transfer': [util.NAN, util.NAN, util.NAN, util.NAN, util.NAN, util.NAN, util.NAN]
                }, dtype=float)
            ]
        }
    })
...
def test_round_by_round(param):
    ...
    table_df = rcv.round_by_round_table(tabulation_num=1)
