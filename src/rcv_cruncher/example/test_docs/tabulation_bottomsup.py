from rcv_cruncher import SingleWinner, STVFractionalBallot, rank_column_csv, BottomsUpThresh
import os

cvr_file = f'{os.path.dirname(__file__)}/../example_cvr/minneapolis2017/2017-mayor-cvr.csv'
out_dir = f'{os.path.dirname(__file__)}/tabulation_bottomsup'
...
election = BottomsUpThresh(
     ...
     exhaust_on_N_repeated_skipped_marks=2,
     n_winners=2,
     writeins_eliminated_first=True,
 )

# save a csv table of the results to out_dir
BottomsUpThresh.write_round_by_round_table(election, out_dir)

# or the json for RCVIS
BottomsUpThresh.write_round_by_round_json(election, out_dir)
