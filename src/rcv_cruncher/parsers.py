...
def cruncher_csv(ctx):
    ...
    Args:
        ctx (dictionary): A dictionary containing key "path" which points to the cvr file containing ranking information of each ballot. Alternatively, if a key "converted_path" is present in the dictionary, it will be preferred to the "path" key. Ranking data must be in columns that follow a "rank#" pattern ("rank1", "rank2", etc). Ranking column must appear in order in the csv file.
    ...
    candidate_codes_fpath = "/".join(cvr_path.split("/")[:-1]) + '/candidate_codes.csv'
    ...
    if 'weight' not in dct:
        ...
        dct['weight'] = [Fraction(str(w)) for w in dct['weight']]
...
def burlington2006(ctx):
    ...
    candidate_codes_fname = "/".join(path.split("/")[:-1]) + "/candidate_codes.csv"
