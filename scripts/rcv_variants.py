...
class rcv_single_winner(RCV):
    ...
    @staticmethod
    def variant_group():
        return RCV.single_winner_group
...
class stv_whole_ballot(stv):
    ...
    def __init__(self, ctx):
        ...
        if len(set(self._cleaned_dict['weight'])) != 1:
            print("ballots with different weights will not work with current implementation.")
    ...
    def _removal_ballots(self, candidate, default_as_true=True):
        ...
        # total and surplus
        init_total = self.get_round_tally_dict(self._round_num, self._tab_num)[candidate]
        ...
        if cand_total > thresh:
            ...
            non_transfer_ballots = [b for idx, b in enumerate(cand_ballots) if idx not in used_idxs]
    ...
    def _clean_round(self):
        ...
        for winner in winners:
            to_remove = self._removal_ballots(winner, default_as_true=True)
